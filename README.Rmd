...
Here we illustrate steps how you can use `FinfoMDS` and obtain 2D representation of your microbiome dataset. As an example, let's take an algal-associated bacterial community (Kim et al., 2022). First, load the data by typing
...
.. where it will iterate for 100 times by default, or until the 2D distributions converge—whichever occurs first. While we have observed that setting `lambda = 0.5` typically yields optimal results, this hyperparameter can be adjusted as long as it does not exceed 1.
