...
.. where it will iterate until the 2D distributions converge or for 100 times by default—whichever occurs first. While we have observed that setting `lambda` between 0.3 and 0.5 typically yields optimal results, this hyperparameter can be adjusted as long as it does not exceed 1.

Finally, the 2D representation of the community dataset is returned as a matrix `result` and can be viewed by typing:
